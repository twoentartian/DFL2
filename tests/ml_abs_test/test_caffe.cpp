...
BOOST_AUTO_TEST_CASE (structure_same_test)
{
	...
	{
        const auto& data_blobs = net1.getLayers()[i].getBlob_p();
        for (const auto& blob : data_blobs) {
            auto& data = blob.get()->getData();
            for (auto&& s : data)
            {
                s = 0;
            }
...
BOOST_AUTO_TEST_CASE (get_model_parameter)
{
    ...
    {
        const auto& data_blobs = parameter1.getLayers()[i].getBlob_p();
        for (const auto& blob : data_blobs) {
            auto& data = blob.get()->getData();
            for (auto&& s : data)
            {
	...
	for (int i = 0; i < parameter1.getLayers().size(); ++i)
	{
		auto& data1 = parameter1.getLayers()[i].getBlob_p();
		auto& data2 = parameter2.getLayers()[i].getBlob_p();
...
{
    Ml::caffe_solver_ext<float, caffe::SGDSolver> model1("../../../dataset/CIFAR10/mobile_net_v2_solver.prototxt");
    BOOST_CHECK(model1.checkValidFirstLayer_memoryLayer());
...
//        MEASURE_TIME(model1.TrainDataset(train_x, train_y));
        ...
        net.fromNet(*model1.net());
        auto v = net.getLayers()[1].getBlob_p()[0]->getData()[0];
        std::cout << v << std::endl;
        BOOST_CHECK(!isnan(v));
...
BOOST_AUTO_TEST_CASE (caffe_cifar_conv_net_train)
{
...
BOOST_AUTO_TEST_CASE (caffe_SimpleNet_Slim_train)
{
    Ml::caffe_solver_ext<float, caffe::SGDSolver> model1("../../../dataset/CIFAR10/SimpleNet_Slim_solver.prototxt");
    BOOST_CHECK(model1.checkValidFirstLayer_memoryLayer());
...
//        MEASURE_TIME(model1.TrainDataset(train_x, train_y));
        ...
        net.fromNet(*model1.net());
        auto v = net.getLayers()[2].getBlob_p()[0]->getData()[0];
        std::cout << v << std::endl;
        BOOST_CHECK(!isnan(v));
...
BOOST_AUTO_TEST_CASE (caffe_ResNet20_train)
{
    Ml::caffe_solver_ext<float, caffe::SGDSolver> model1("../../../dataset/CIFAR10/ResNet20_solver.prototxt");
    BOOST_CHECK(model1.checkValidFirstLayer_memoryLayer());
