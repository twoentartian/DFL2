				...
				for (auto& [single_transaction_hash, single_transaction]: blk.content.transaction_container)
				{
					...
					for (auto& [receipt_hash, receipt]: single_transaction.receipts)
					{
						...
						if (receipt.content.creator.node_address == _address.getTextStr_lowercase())
						{
							...
							auto result_check_verified_transaction = _main_transaction_storage_for_block->check_verified_transaction(single_transaction);
							if (result_check_verified_transaction == transaction_storage_for_block::check_receipt_return::pass)
							{
								//remove this transaction
								_main_transaction_storage_for_block->remove_verified_transaction(single_transaction);
							...
							else
							{
								...
								LOG(WARNING) << "check verified transaction failed, status: " << transaction_storage_for_block::to_string(result_check_verified_transaction);
							}
						}
					}
				}
